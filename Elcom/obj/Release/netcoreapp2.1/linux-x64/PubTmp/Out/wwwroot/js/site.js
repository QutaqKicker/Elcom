...
function showTable(tableID, actionID, listClassName, actionClassName, caption)
{
    var elementsList = document.getElementsByClassName(listClassName);
    for(var i = 0; i < elementsList.length; i++)
    {
        if (elementsList[i].style.display == 'block')
            elementsList[i].style.display = 'none';
    }
    document.getElementById(tableID).style.display = 'block';
    document.getElementById('heading').innerHTML = caption;

    var elementsAction = document.getElementsByClassName(actionClassName);
    for(var i = 0; i < elementsAction.length; i++)
    {
        if (elementsAction[i].style.display == 'block')
            elementsAction[i].style.display = 'none';
    }
    document.getElementById(actionID).style.display = 'block';

    if(tableID == 'DelayedPositions'){
        var upddates = document.getElementsByClassName('updateDates');
        for(var i = 0; i < upddates.length; i++)
        {
            if (upddates[i].textContent == 'Обновите сроки ЗК')
                upddates[i].style.color = '#FF0000';
